...
describe('testing productos',() => {
    ...
    describe('test crea un producto', () => {
        it('El endpoint POST /api/products/ debe crear exitosamente un nuevo producto',
        async () => {
                const productMock = {
    ...
    describe('test de eliminar producto', () => {
        it('El endpoint DELETE /api/products/:pid debe eliminar el producto recien creado',
        async () => {
            const { statusCode, _body, ok } = await requester.delete(`/api/products/${prodId}`);
            expect(statusCode).to.be.equal(200);
